...
public final class ConditionProgramCompiler {
    ...
    public static DirectConditionExecutor compile(CompiledQuery.CompiledCondition condition, boolean primitiveNonNull) {
    ...
    private static DirectConditionExecutor compileEq(int columnIndex, int argumentIndex, boolean ignoreCase,
        ...
        return switch (typeCode) {
            case TypeCodes.TYPE_STRING,
                    ...
                    TypeCodes.TYPE_BIG_INTEGER -> ignoreCase
                            ? (table, kernel, args) -> selectionFromRows(table, table.scanEqualsStringIgnoreCase(
                                    columnIndex,
                                    ConditionArgDecoders.toStringValue(ConditionArgDecoders.argAt(args, argumentIndex))))
                            : (table, kernel, args) -> selectionFromRows(table, table.scanEqualsString(columnIndex,
                                    ConditionArgDecoders.toStringValue(ConditionArgDecoders.argAt(args,
                                            argumentIndex))));
            case TypeCodes.TYPE_LONG,
                    ...
                    TypeCodes.TYPE_DATE,
                    TypeCodes.TYPE_DOUBLE -> (table, kernel, args) -> selectionFromRows(table,
            ...
            default -> (table, kernel, args) -> selectionFromRows(table, table.scanEqualsInt(columnIndex,
                    ConditionArgDecoders.toInt(typeCode, ConditionArgDecoders.argAt(args, argumentIndex))));
        };
    }
    ...
    private static DirectConditionExecutor compileNe(int columnIndex, int argumentIndex, boolean ignoreCase,
        ...
        return (table, kernel, args) -> selectionFromRows(table, table.scanAll()).subtract(eq.execute(table, kernel, args));
    }
            ...
            case TypeCodes.TYPE_LONG,
                    ...
                    TypeCodes.TYPE_DOUBLE -> (table, kernel, args) -> selectionFromRows(table,
                            table.scanBetweenLong(columnIndex,
                                    ConditionArgDecoders.toLong(typeCode, ConditionArgDecoders.argAt(args, argumentIndex))
                                            + 1L,
                                    Long.MAX_VALUE));
            default -> (table, kernel, args) -> selectionFromRows(table,
                    table.scanBetweenInt(columnIndex,
                            ConditionArgDecoders.toInt(typeCode, ConditionArgDecoders.argAt(args, argumentIndex)) + 1,
                            Integer.MAX_VALUE));
        };
    }
            ...
            case TypeCodes.TYPE_LONG,
                    ...
                    TypeCodes.TYPE_DOUBLE -> (table, kernel, args) -> selectionFromRows(table,
                            table.scanBetweenLong(columnIndex,
                                    ConditionArgDecoders.toLong(typeCode, ConditionArgDecoders.argAt(args, argumentIndex)),
                                    ConditionArgDecoders.toLong(typeCode,
                                            ConditionArgDecoders.argAt(args, argumentIndex + 1))));
            default -> (table, kernel, args) -> selectionFromRows(table,
                    table.scanBetweenInt(columnIndex,
                            ConditionArgDecoders.toInt(typeCode, ConditionArgDecoders.argAt(args, argumentIndex)),
                            ConditionArgDecoders.toInt(typeCode, ConditionArgDecoders.argAt(args, argumentIndex + 1))));
    ...
    private static DirectConditionExecutor compileIn(int columnIndex, int argumentIndex, byte typeCode) {
        return switch (typeCode) {
            case TypeCodes.TYPE_STRING,
                    TypeCodes.TYPE_BIG_DECIMAL,
                    TypeCodes.TYPE_BIG_INTEGER -> (table, kernel, args) -> selectionFromRows(table,
                            table.scanInString(columnIndex,
                                    ConditionArgDecoders.toStringArray(ConditionArgDecoders.argAt(args, argumentIndex))));
            case TypeCodes.TYPE_LONG,
                    TypeCodes.TYPE_INSTANT,
                    TypeCodes.TYPE_LOCAL_DATE,
                    TypeCodes.TYPE_LOCAL_DATE_TIME,
            ...
            default -> (table, kernel, args) -> selectionFromRows(table,
                    table.scanInInt(columnIndex,
                            ConditionArgDecoders.toIntArray(typeCode, ConditionArgDecoders.argAt(args, argumentIndex))));
        };
    }
    ...
    private static DirectConditionExecutor compileNotIn(int columnIndex, int argumentIndex, byte typeCode) {
        ...
        return (table, kernel, args) -> selectionFromRows(table, table.scanAll()).subtract(in.execute(table, kernel, args));
    }
