...
public class QueryCompiler {
    ...
    private boolean compileJoinPath(
        ...
        while (idx < segments.length - 1) {
            ...
            if (fkTypeCode != io.memris.spring.TypeCodes.TYPE_LONG
                && fkTypeCode != io.memris.spring.TypeCodes.TYPE_INT
                && fkTypeCode != io.memris.spring.TypeCodes.TYPE_SHORT
                && fkTypeCode != io.memris.spring.TypeCodes.TYPE_BYTE) {
    ...
    private EntityMetadata<?> resolveMetadata(Class<?> entityClass) {
        return metadataCache.computeIfAbsent(entityClass, io.memris.spring.MetadataExtractor::extractEntityMetadata);
    }
}
