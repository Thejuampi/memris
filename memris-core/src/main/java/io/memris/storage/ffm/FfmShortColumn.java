...
public final class FfmShortColumn {
    ...
    public FfmShortColumn(String name, Arena arena, int capacity) {
        ...
        long bytes = (long) capacity * ValueLayout.JAVA_SHORT.byteSize();
        this.name = name;
        this.segment = arena.allocate(bytes, ValueLayout.JAVA_SHORT.byteAlignment());
    ...
    public SelectionVector scanEquals(short value, int rowCount, SelectionVectorFactory factory) {
        ...
        for (; i < loopBound; i += SPECIES.length()) {
            IntVector vector = IntVector.fromMemorySegment(SPECIES, segment,
                    (long) i * ValueLayout.JAVA_SHORT.byteSize(), ByteOrder.nativeOrder());
        ...
        for (; i < rowCount; i++) {
            if (segment.getAtIndex(ValueLayout.JAVA_SHORT, i) == value) {
                selection.add(i);
            }
        }
        return selection;
