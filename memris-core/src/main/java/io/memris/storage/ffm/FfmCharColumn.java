...
public final class FfmCharColumn {
    ...
    public FfmCharColumn(String name, Arena arena, int capacity) {
        ...
        long bytes = (long) capacity * ValueLayout.JAVA_CHAR.byteSize();
        this.name = name;
        this.segment = arena.allocate(bytes, ValueLayout.JAVA_CHAR.byteAlignment());
    ...
    public SelectionVector scanEquals(char value, int rowCount, SelectionVectorFactory factory) {
        ...
        for (; i < loopBound; i += SPECIES.length()) {
            IntVector vector = IntVector.fromMemorySegment(SPECIES, segment,
                    (long) i * ValueLayout.JAVA_CHAR.byteSize(), ByteOrder.nativeOrder());
        ...
        for (; i < rowCount; i++) {
            if (segment.getAtIndex(ValueLayout.JAVA_CHAR, i) == value) {
                selection.add(i);
            }
        }
        return selection;
