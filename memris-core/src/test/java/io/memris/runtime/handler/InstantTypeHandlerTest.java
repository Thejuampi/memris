...
class InstantTypeHandlerTest {
    private static final InstantTypeHandler HANDLER = new InstantTypeHandler();
    ...
    @Test void typeCodeAndJavaType() {
        ...
        assertThat(HANDLER.getJavaType()).isEqualTo(Instant.class);
    }

    @Test void convertValueAcceptsInstant() {
        Instant d = Instant.ofEpochMilli(1234L);
        assertThat(HANDLER.convertValue(d)).isSameAs(d);
    }

    @Test void convertValueRejectsNonInstant() {
        assertThatThrownBy(() -> HANDLER.convertValue("not an instant")).isInstanceOf(IllegalArgumentException.class);
    }

    @Test void executeEqualsFindsMatching() {
        Instant d = Instant.ofEpochMilli(1000L);
    ...
    @Test void executeLessThan() {
        Instant d = Instant.ofEpochMilli(1500L);
        TestTable t = new TestTable(List.of(500L, 1000L, 1500L, 2000L));
