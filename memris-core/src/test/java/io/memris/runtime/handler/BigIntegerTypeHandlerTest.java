...
class BigIntegerTypeHandlerTest {
    private static final BigIntegerTypeHandler HANDLER = new BigIntegerTypeHandler();
    ...
    @Test void typeCodeAndJavaType() {
        ...
        assertThat(HANDLER.getJavaType()).isEqualTo(BigInteger.class);
    }

    @Test void convertValueAcceptsBigInteger() {
        BigInteger b = new BigInteger("12345678901234567890");
        assertThat(HANDLER.convertValue(b)).isSameAs(b);
    }

    @Test void convertValueRejectsNonBigInteger() {
        assertThatThrownBy(() -> HANDLER.convertValue("not a biginteger")).isInstanceOf(IllegalArgumentException.class);
    }

    @Test void executeEqualsFindsMatching() {
        BigInteger b = new BigInteger("123");
        TestTable t = new TestTable(List.of("1", "123", "999"));
