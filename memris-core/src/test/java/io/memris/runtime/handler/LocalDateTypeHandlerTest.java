...
class LocalDateTypeHandlerTest {
    private static final LocalDateTypeHandler HANDLER = new LocalDateTypeHandler();
    ...
    @Test void typeCodeAndJavaType() {
        ...
        assertThat(HANDLER.getJavaType()).isEqualTo(LocalDate.class);
    }

    @Test void convertValueAcceptsLocalDate() {
        LocalDate d = LocalDate.ofEpochDay(1234L);
        assertThat(HANDLER.convertValue(d)).isSameAs(d);
    }

    @Test void convertValueRejectsNonLocalDate() {
    ...
    @Test void executeEqualsFindsMatching() {
        LocalDate d = LocalDate.ofEpochDay(1000L);
    ...
    @Test void executeLessThan() {
        LocalDate d = LocalDate.ofEpochDay(1500L);
        TestTable t = new TestTable(List.of(500L, 1000L, 1500L, 2000L));
